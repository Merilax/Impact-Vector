...
func refresh():
	...
	var base_campaigns:Array = JSON.parse_string(base_json)

	for campaign in base_campaigns:
		var campaign_box:CampaignBox = CampaignBoxScene.instantiate()
		campaign_box.get_node("HBoxContainer/Name").text = campaign.name
		campaign_box.set_meta("folder_path", "res://Levels/")
		campaign_box.set_meta("folder_num", campaign.dir)
	...
	var custom_campaigns:Array = JSON.parse_string(custom_json)
